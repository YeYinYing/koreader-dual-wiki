...
-- matrix (zh) plus Phase 2.1 cross-language cases (ja/en) and mishit guards.
local src_path = arg[1] or "dual_wiki.koplugin/main.lua"
...
local function check(name, got, want)
    if got == want then
        print(string.format("PASS  %-32s got=%q", name, tostring(got)))
    else
        print(string.format("FAIL  %-32s got=%q want=%q", name, tostring(got), tostring(want)))
        failures = failures + 1
    end
end
...
check("N7 de 默认回 zh", api.normalizeLang("de-DE"), "zh")

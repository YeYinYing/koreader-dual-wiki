# KOReader Dual Wiki Plugin (`dual_wiki.koplugin`)
...
1. **双独立并列入口 (Dual Independent Entries)**：
   - 划词高亮工具栏中并列呈现 **`【萌娘百科】`** 与 **`【维基百科】`** 两个独立按钮；
   - **零侵入架构**：纯粹运行于插件沙盒，无需打补丁或修改 KOReader 核心系统文件。
2. **墨水屏黑幕自动全透视 (Spoiler Bar Revelation)**：
   - 针对萌娘百科特色的 `#252525` 黑色恶搞/剧透遮罩（黑幕）：查询管道全程使用 `explaintext=1` 纯文本模式，黑幕作为纯 CSS 背景效果根本不会进入显示层——所有被遮盖的文字天然呈现为清晰易读的纯正中文，墨水屏上绝无实心黑块。
3. **彻底解除篇幅腰斩限制 (Full-Length Uncapped Articles)**：
   - 告别官方原生维基强制只给看两句摘要（`exintro`）的割裂体验。单次请求一次性完整载入整篇数千字至上万字长文（生平、年表、全剧情推演），支持顺畅翻页。
4. **毫秒级纯文本直出管道 (Sub-second Pipeline: 0.4s - 0.8s)**：
   - 彻底剔除图片缩略图抓取（`pageimages`），杜绝在弱网墨水屏设备上因图片加载导致的 SSL 握手超时与 HTTP 429 限流卡死。
5. **实体书级章节标题重排 (Book-grade Typography)**：
   - 自动将维基语法标记（`== 标题 ==` 与 `=== 子节 ===`）转化为出版级排版的 **`【主章节】`** 与 **`▸ 子章节`**。
6. **双擎跨引擎一键互切与关键词微调 (Cross-Engine Switching & Calibration)**：
...
### 方式二：KOReader AppStore 在线安装
在设备端打开 `appstore.koplugin`（应用商店），搜索 **Dual Wiki**，点击安装即可。

---

## ⚙️ 架构规范与兼容性 (Architecture)
...
- **设备验证**：已在 Kindle Paperwhite 3 (i.MX6SL, 512MB RAM) 实机深度验证，内存零常驻，弱网高容错。
